...
After AC, require teach-back covering the invariant, complexity, an easy-to-miss edge case, and when the pattern does not apply. The conversational decision engine owns intent: emit `teach_back` only when the learner is actually supplying that evidence, and keep unrelated questions as normal `continue` turns. Never claim a write succeeded until the graph confirms persistence.
...
Normalize UI/provider content blocks into plain learner text before interpreting commands. `TurnDecision` represents conversational intent; `TeachBackDecision` evaluates evidence only after a `teach_back` event. Derive completion deterministically from invariant, complexity, edge-case, and pattern-boundary evidence; response prose is never a write. Once complete, build one `complete_attempt` preview containing archive, attempt, sweep, and session changes. Require one approval and roll all protected files back if any grouped write fails.
