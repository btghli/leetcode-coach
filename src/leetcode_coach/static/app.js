...
function overview() {
  ...
  const hasActiveProblem=Boolean(conversation.selected_problem && conversation.phase !== 'complete');
  ...
  <section class="hero"><div><small>下一步 · ${hasActiveProblem ? '继续当前练习' : '学习计划推荐'}</small><h2>${esc(conversation.selected_problem?.title || p?.title || '准备开始练习')}</h2><span class="muted">${p?.reason==='due-review'?'到期复习 · 尝试从记忆中独立推导':'根据你的本地学习记录安排'}</span></div><a class="button primary" href="#training">进入训练 →</a></section>
  <section class="panel lesson"><div class="row"><div><div class="eyebrow">TODAY'S LESSON</div><h2>今日课程：${target.review||0} 道复习 · ${target.new||0} 道新题</h2><p class="muted">先处理到期复习，再进入新题；进度始终从本地学习记录重新计算。</p></div><button class="primary" ${hasActiveProblem?'data-resume':'data-start'}>${hasActiveProblem?'继续当前题':'开始今日课程'} →</button></div><div class="lesson-queue">${lessonItems.length?lessonItems.map((item,i)=>`<div class="item"><span class="badge">${i+1}</span> <strong>${esc(item.kind)}</strong> · #${esc(item.id||'—')} ${esc(item.title||item.slug)}<br><small>${esc(item.difficulty||'')}${item.next_review?` · 复习日期 ${esc(item.next_review)}`:''}</small></div>`).join(''):'<p class="muted">今日计划已完成，或还没有可用题目。</p>'}</div></section>
...
function training() {
  ...
  document.querySelector('#judge').onchange=e=>{if(e.target.value)send({action:'judge',result:e.target.value});};
...
function setBusy(){document.querySelectorAll('#composer button,#composer select,#composer textarea,[data-start],[data-resume],[data-next],[data-decision],[data-new],[data-history],[data-refresh]').forEach(b=>b.disabled=busy);const w=document.querySelector('#working');if(w)w.textContent=busy?'Coach 正在处理，请稍候…':'Enter 换行 · Ctrl/⌘ + Enter 发送';}
...
document.addEventListener('click',async e=>{const b=e.target.closest('button');if(!b)return;try{
 ...
 if(b.hasAttribute('data-resume'))location.hash='training';
 if(b.hasAttribute('data-start')){location.hash='training';await send({action:'start'});}
 if(b.hasAttribute('data-next'))await send({action:'next'});
