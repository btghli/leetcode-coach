"""Canonical study-data commands and compatibility operations."""
...
def review_days(
    *,
    ...
    if review_in_days is not None:
        return review_in_days
    if status == "Todo":
        return None
    if status in {"Doing", "Review"}:
        return 1
    if mastery == "solid":
        days = 30
    elif mastery == "ok":
        ...
        days = 1
    quality = quality if quality is not None else stats.get("recall_score")
    hint = hint_level if hint_level is not None else stats.get("hint_level_reached")
    try:
    ...
    if quality is not None:
        if quality <= 2:
            ...
            days = min(days, 3 if mastery != "solid" else 7)
        elif quality == 5 and mastery == "ok":
            days = max(days, 14)
    if hint >= 4:
        days = min(days, 1)
    elif hint >= 3:
...
def initialize_problem(
    ...
    *,
    ...
    if find_problem(root, slug) and not force:
        raise ValueError(f"Problem already exists for slug={slug}. Use force to overwrite metadata/template.")
    found_by_id = find_problem_by_id(root, problem_id)
    if found_by_id and found_by_id[1].get("slug") != slug and not force:
        raise ValueError(f"Problem id={problem_id} already exists as slug={found_by_id[1].get('slug')}.")
    slug = slug.strip()
    meta = normalize_meta({
        "id": int(problem_id),
        "slug": slug,
    ...
    dest_dir = problem_dir(root, int(problem_id), slug)
    dest_dir.mkdir(parents=True, exist_ok=True)
    note = dest_dir / "note.md"
    if note.exists() and not force:
        raise ValueError(f"note.md already exists: {note}. Use force to overwrite.")
    ...
    return {"note": str(note), "solution": str(solution), "metadata": meta}


def command_init_problem(args: argparse.Namespace) -> int:
    root = root_from_args(args)
    try:
        result = initialize_problem(
            root,
            problem_id=args.id,
            slug=args.slug,
            title=args.title,
    ...
    if args.json:
        ...
        print(f"Initialized: {result['note']}")
        print(f"Solution placeholder: {result['solution']}")
    return 0
...
def finish_attempt(
    ...
    *,
    ...
    solve_minutes: Optional[int] = None,
    first_try_ac: Optional[bool] = None,
    judge_failures: Optional[Iterable[str]] = None,
    teach_back: Optional[bool] = None,
    allow_unverified_solid: bool = False,
    clear_mistake_tags: bool = False,
    mistake_tags: Iterable[str] = (),
    next_review: Optional[str] = None,
    review_in_days: Optional[int] = None,
) -> Dict[str, Any]:
    found = find_problem(root, slug)
    if not found:
        raise ValueError(f"Unknown problem slug={slug}. Run init-problem first.")
    path, meta = found
    prof = profile(root)
    stats = normalize_stats(meta.get("stats"))
    stats["attempts"] = int(stats.get("attempts") or 0) + 1
    if hint_level is not None:
        stats["hint_level_reached"] = int(hint_level)
    if solve_minutes is not None:
        stats["solve_minutes"] = int(solve_minutes)
    if first_try_ac is not None:
        stats["first_try_ac"] = bool(first_try_ac)
    if judge_failures is not None:
        stats["judge_failures"] = unique(x.upper() for x in judge_failures)
    if quality is not None:
        stats["recall_score"] = int(quality)
    if teach_back is not None:
        stats["teach_back_done"] = bool(teach_back)
    if mode:
    ...
    if (
        ...
        and not allow_unverified_solid
        and not bool(stats.get("teach_back_done"))
    ):
        raise ValueError("Refusing mastery=solid: teach_back_done is false.")
    old_mistakes = [] if clear_mistake_tags else list(meta.get("mistake_tags", []))
    new_mistakes = list(mistake_tags)
    repeated = bool(set(old_mistakes) & set(new_mistakes))
    mistakes = unique(old_mistakes + new_mistakes)
    next_review: Optional[str]
    if next_review:
        parse_date(next_review)  # validate
    else:
        days = review_days(
            status=status,
            mastery=mastery,
...
def command_finish(args: argparse.Namespace) -> int:
    ...
    try:
        updated = finish_attempt(
            ...
            solve_minutes=args.solve_minutes,
            first_try_ac=args.first_try_ac,
            judge_failures=split_csv(args.judge_failures) if args.judge_failures is not None else None,
            teach_back=args.teach_back,
            allow_unverified_solid=args.allow_unverified_solid,
            clear_mistake_tags=args.clear_mistake_tags,
    ...
    if args.json:
        ...
        print(f"status={args.status} mastery={args.mastery} next_review={updated.get('next_review')} attempts={stats['attempts']}")
        if updated.get("mistake_tags"):
            print("mistake_tags=" + ",".join(updated["mistake_tags"]))
    return 0
...
def command_log_session(args: argparse.Namespace) -> int:
    ...
    if args.json:
        ...
        print(f"Logged session entry: {result['session']}")
    return 0
...
def archive_solution(
    ...
    *,
    ...
    source_path: Optional[Path] = Path(source).expanduser().resolve() if source else None
    if source_path is None:
        matches = matching_plugin_files(root, slug, meta.get("id")) if from_plugin else []
        if not matches:
            raise ValueError("No source solution found. Provide --source PATH or use --from-plugin after submitting in VS Code.")
        source_path = Path(matches[0]["path"])
    if not source_path.exists():
        raise ValueError(f"Source does not exist: {source_path}")
    code = extract_code(source_path)
    if mode == "standalone":
...
def command_archive_solution(args: argparse.Namespace) -> int:
    ...
    if args.json:
        ...
        print(f"Archived solution: {result['dest']}")
        print(f"Source: {result['source']}")
        print(f"Mode: {result['mode']}")
        if result["test"]:
            print(f"Test skeleton: {result['test']}")
    return 0
